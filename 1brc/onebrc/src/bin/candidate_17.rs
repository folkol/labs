...
impl<'a> NameTable<'a> {
    ...
    /// - Never copies `Entry` out of `self.entries` (uses refs).
    /// - Avoids creating `&[u8]` slices on the slow-path by comparing via raw pointers.
    /// - Uses `get_unchecked_mut` for the returned stats to avoid an extra bounds check.
    #[inline(always)]
    pub fn get_or_insert_stats(&mut self, name_off: u64, name_len: u16) -> &mut StationStats {
        ...
        loop {
            ...
            // IMPORTANT: do not copy the Entry.
            let e = unsafe { self.entries.get_unchecked(entry_idx) };

            // Fast accept/reject on (hash, len, first 16B).
            if e.hash == h && e.name_len == name_len && e.w1 == w1 && e.w2 == w2 {
                if len <= 16 {
                    return unsafe { &mut self.entries.get_unchecked_mut(entry_idx).stats };
                }

                // Rare slow path: compare the full key bytes.
                // Avoid slice creation: compare raw pointers directly.
    ...
    unsafe fn bytes_eq_u64_ptr(mut a: *const u8, mut b: *const u8, mut len: usize) -> bool { unsafe {
        ...
        if len != 0 {
            // compare remaining 1..7 bytes via masked u64 load
