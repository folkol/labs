...
struct Entry {
    name_off: u64,
    name_len: u16,
    // cached prefix (masked for short keys)
    w1: u64,
    w2: u64,
    hash: u64,
    stats: StationStats,
...
#[inline]
fn prefix16(key: &[u8]) -> (u64, u64) {
    let w1 = read_u64_prefix(key);
    let w2 = if key.len() > 8 { read_u64_prefix(&key[8..]) } else { 0 };
    (w1, w2)
}
...
#[inline]
fn hash_prefix(w1: u64, w2: u64, len: u16) -> u64 {
    mix64(w1 ^ w2.rotate_left(17) ^ (len as u64).wrapping_mul(0x9e3779b97f4a7c15))
}
...
impl<'a> NameTable<'a> {
    ...
    pub fn get_or_insert_stats(&mut self, name_off: u64, name_len: u16) -> &mut StationStats {
        ...
        let (w1, w2) = prefix16(key);
        let h = hash_prefix(w1, w2, name_len);
        ...
        loop {
            ...
            // very cheap equality for most names
            if e.hash == h && e.name_len == name_len && e.w1 == w1 && e.w2 == w2 {
                // only if names can exceed 16 bytes, fall back:
                if name_len as usize <= 16 || self.key_bytes(e.name_off, e.name_len) == key {
