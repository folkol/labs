...
impl<'a> NameTable<'a> {
    const MAX_LOAD: f32 = 0.60;

    #[inline]
    fn should_grow(&self) -> bool {
        // entries / slots
        (self.entries.len() as f32) >= (self.slots.len() as f32) * Self::MAX_LOAD
    }

    fn resize_and_rehash(&mut self, new_slots_len: usize) {
        assert!(new_slots_len.is_power_of_two());
        let mut new_slots = vec![0u32; new_slots_len];
        let new_mask = new_slots_len - 1;
        let step = 31usize;

        for (idx, e) in self.entries.iter().enumerate() {
            // same index_for logic, but with new_mask
            let mut i = {
                let x = e.hash ^ (e.hash >> 33);
                (x as usize) & new_mask
            };

            loop {
                let slot = new_slots[i];
                if slot == 0 {
    ...
    pub fn with_capacity(data: &'a [u8], capacity: usize) -> Self {
        let desired = ((capacity as f64) / (Self::MAX_LOAD as f64)).ceil() as usize;
    ...
    pub fn get_or_insert_stats(&mut self, name_off: u64, name_len: u16) -> &mut StationStats {
        ...
        loop {
            ...
            if slot == 0 {
                if self.should_grow() {
                    // grow to next power of two; doubling is fine
                    self.resize_and_rehash(self.slots.len() * 2);
                    // restart probe with the new table
                    i = self.index_for(h);
                    continue;
                }
...
fn chunk_statistics(data: &[u8], chunk_start: usize, chunk_end: usize, statistics: &mut NameTable) {
    ...
    for end in memchr::memchr_iter(b'\n', data) {
        ...
        let temperature = parse_temp_tenths_fixed_dot(data, end + 1);

        let entry = statistics
            .get_or_insert_stats((chunk_start + start) as u64, (semicolon - start) as u16);
        entry.count += 1;
