...
const fn page_trunc(v: usize) -> usize {
    v & PAGE_MASK
}
const fn huge_page_trunc(v: usize) -> usize {
    v & HUGE_PAGE_MASK
}
const fn page_ceil(v: usize) -> usize {
    page_trunc(v + PAGE_SIZE - 1)
}
const fn huge_page_ceil(v: usize) -> usize {
    huge_page_trunc(v + HUGE_PAGE_SIZE - 1)
}

const LINE_SIZE: usize = 64;
const LINE_MASK: usize = !(LINE_SIZE - 1);
const fn line_trunc(v: usize) -> usize {
    v & LINE_MASK
}
const fn line_ceil(v: usize) -> usize {
    line_trunc(v + LINE_SIZE - 1)
}
...
const HASH_MEMORY_SIZE: usize =
    PACKED_OFFSETS_SIZE + HASHED_CITIES_SIZE + HASHED_DATA_SIZE + HASHED_CITIES_LONG_SIZE;
...
const RESULTS_MEMORY_SIZE: usize =
    page_ceil(RESULTS_SIZE + RESULTS_REFS_SIZE + RESULTS_ROWS_SIZE + RESULTS_LONG_CITIES_SIZE);
...
fn extract_count(v: i64) -> i32 {
    (v >> COUNT_BITS_START) as i32
}
const SUM_MASK: i64 = (1 << COUNT_BITS_START) - 1;
fn extract_sum(v: i64) -> i64 {
    (v & SUM_MASK) - SUM_SIGN_BIT
}
...
fn main() {
    unsafe {
        ...
        let warmup = if args.len() == 4 {
            args[3].parse::<i32>().unwrap_or(0) != 0
        } else {
            false
        };
        ...
        qsort_r(
            (*results).refs as *mut c_void,
            (*results).num_cities as usize,
            size_of::<ResultsRef>(),
            Some(sort_result),
...
unsafe fn prep_workers(
    workers: *mut Worker,
    num_workers: i32,
    warmup: bool,
    fd: i32,
    file_stat: *const stat,
) {
    ...
    if num_cpus < num_workers {
        eprintln!(
            "{} threads is less than {} available CPUS",
...
unsafe fn process(
    ...
    unsafe {
        ...
        let tmp = mmap(
            ptr::null_mut(),
            RESULTS_MEMORY_SIZE * k as usize,
            PROT_READ | PROT_WRITE,
            ...
            if pid == 0 {
                ...
                if do_work {
                    if PIN_CPU {
                        ...
                        CPU_SET(
                            (*workers.add(new_id as usize)).cpu_id as usize,
                            &mut cpu_set,
                        );
                ...
                process(
                    ...
                    workers,
                    n,
                    fd_pipes[i as usize][1],
                    child_results[i as usize],
                );
                if UNMAP {
        ...
        while children_finished < k {
            ...
            for i in 0..k {
                if poll_fds[i as usize].fd == -1 {
                    continue;
                }
                if (poll_fds[i as usize].revents & POLLIN) != 0 {
                    let mut buffer = [0u8; 4];
                    let num_bytes = read(
                        poll_fds[i as usize].fd,
                ...
                } else if (poll_fds[i as usize].revents
...
unsafe fn start_worker(w: *mut Worker, out: *mut Results) {
    unsafe {
        let mut hash_data = mmap(
            ptr::null_mut(),
            HASH_MEMORY_SIZE + HUGE_PAGE_SIZE,
            PROT_READ | PROT_WRITE,
            MAP_PRIVATE | MAP_ANONYMOUS,
            -1,
            0,
        ...
        madvise(
            hash_data.add(PACKED_OFFSETS_SIZE),
            HASHED_CITIES_SIZE + HASHED_DATA_SIZE + HASHED_CITIES_LONG_SIZE,
            MADV_HUGEPAGE,
        );
        ...
        while start < (*w).end {
            ...
            let mapped_file_length = if last {
                page_ceil(chunk_size as usize)
            } else {
                chunk_size as usize + PAGE_SIZE
            };
            ...
            for i in (if first { 1 } else { 0 })..STRIDE {
                offsets[i] = find_next_row(
                    data,
                    (chunk_size / STRIDE as u32 * i as u32) + DUMMY_SIZE as u32,
                );
            }
            offsets[STRIDE] = if last {
                chunk_size + DUMMY_SIZE as u32
            } else {
                find_next_row(data, chunk_size + DUMMY_SIZE as u32)
            };
...
unsafe fn find_next_row(data: *const c_void, offset: u32) -> u32 {
    ...
    let bytes =
        _mm_tzcnt_32(_mm256_movemask_epi8(_mm256_cmpeq_epi8(chars, newlines)) as u32) as u32;
    if bytes < 32 {
...
unsafe fn process_chunk(base: *const c_void, offsets: *const u32, hash_out: *mut Hash) {
    ...
    _mm256_store_si256(
        hash.p.hashed_cities.add(hash_city(dummy) as usize) as *mut __m256i,
        dummy,
    );
    ...
    loop {
        if check_finished {
            ...
            ends_v = _mm256_castps_si256(_mm256_blendv_ps(
                _mm256_castsi256_ps(ends_v),
                _mm256_castsi256_ps(_mm256_set1_epi32(DUMMY_SIZE as i32)),
                _mm256_castsi256_ps(finished_v),
            ));
        ...
        let mut sc0 =
            _mm_tzcnt_32(_mm256_movemask_epi8(_mm256_cmpeq_epi8(raw_city0, semicolons)) as u32)
                as i32;
        let mut sc1 =
            _mm_tzcnt_32(_mm256_movemask_epi8(_mm256_cmpeq_epi8(raw_city1, semicolons)) as u32)
                as i32;
        let mut sc2 =
            _mm_tzcnt_32(_mm256_movemask_epi8(_mm256_cmpeq_epi8(raw_city2, semicolons)) as u32)
                as i32;
        let mut sc3 =
            _mm_tzcnt_32(_mm256_movemask_epi8(_mm256_cmpeq_epi8(raw_city3, semicolons)) as u32)
                as i32;
        let mut sc4 =
            _mm_tzcnt_32(_mm256_movemask_epi8(_mm256_cmpeq_epi8(raw_city4, semicolons)) as u32)
                as i32;
        let mut sc5 =
            _mm_tzcnt_32(_mm256_movemask_epi8(_mm256_cmpeq_epi8(raw_city5, semicolons)) as u32)
                as i32;
        let mut sc6 =
            _mm_tzcnt_32(_mm256_movemask_epi8(_mm256_cmpeq_epi8(raw_city6, semicolons)) as u32)
                as i32;
        let mut sc7 =
            _mm_tzcnt_32(_mm256_movemask_epi8(_mm256_cmpeq_epi8(raw_city7, semicolons)) as u32)
                as i32;
        ...
        let masked_city0 = _mm256_and_si256(
            raw_city0,
            _mm256_loadu_si256(CITY_MASK.0.as_ptr().add(32 - sc0 as usize) as *const __m256i),
        );
        let masked_city1 = _mm256_and_si256(
            raw_city1,
            _mm256_loadu_si256(CITY_MASK.0.as_ptr().add(32 - sc1 as usize) as *const __m256i),
        );
        let masked_city2 = _mm256_and_si256(
            raw_city2,
            _mm256_loadu_si256(CITY_MASK.0.as_ptr().add(32 - sc2 as usize) as *const __m256i),
        );
        let masked_city3 = _mm256_and_si256(
            raw_city3,
            _mm256_loadu_si256(CITY_MASK.0.as_ptr().add(32 - sc3 as usize) as *const __m256i),
        );
        let masked_city4 = _mm256_and_si256(
            raw_city4,
            _mm256_loadu_si256(CITY_MASK.0.as_ptr().add(32 - sc4 as usize) as *const __m256i),
        );
        let masked_city5 = _mm256_and_si256(
            raw_city5,
            _mm256_loadu_si256(CITY_MASK.0.as_ptr().add(32 - sc5 as usize) as *const __m256i),
        );
        let masked_city6 = _mm256_and_si256(
            raw_city6,
            _mm256_loadu_si256(CITY_MASK.0.as_ptr().add(32 - sc6 as usize) as *const __m256i),
        );
        let masked_city7 = _mm256_and_si256(
            raw_city7,
            _mm256_loadu_si256(CITY_MASK.0.as_ptr().add(32 - sc7 as usize) as *const __m256i),
        );
        ...
        if _mm256_testz_si256(long_cities, long_cities) == 0 {
            if sc0 == 32 {
                masked_city0 = process_long(base.add(starts[0] as usize), &mut hash, &mut sc0);
                semicolons_v = _mm256_insert_epi32(semicolons_v, sc0, 0);
            }
            if sc1 == 32 {
                masked_city1 = process_long(base.add(starts[1] as usize), &mut hash, &mut sc1);
                semicolons_v = _mm256_insert_epi32(semicolons_v, sc1, 1);
            }
            if sc2 == 32 {
                masked_city2 = process_long(base.add(starts[2] as usize), &mut hash, &mut sc2);
                semicolons_v = _mm256_insert_epi32(semicolons_v, sc2, 2);
            }
            if sc3 == 32 {
                masked_city3 = process_long(base.add(starts[3] as usize), &mut hash, &mut sc3);
                semicolons_v = _mm256_insert_epi32(semicolons_v, sc3, 3);
            }
            if sc4 == 32 {
                masked_city4 = process_long(base.add(starts[4] as usize), &mut hash, &mut sc4);
                semicolons_v = _mm256_insert_epi32(semicolons_v, sc4, 4);
            }
            if sc5 == 32 {
                masked_city5 = process_long(base.add(starts[5] as usize), &mut hash, &mut sc5);
                semicolons_v = _mm256_insert_epi32(semicolons_v, sc5, 5);
            }
            if sc6 == 32 {
                masked_city6 = process_long(base.add(starts[6] as usize), &mut hash, &mut sc6);
                semicolons_v = _mm256_insert_epi32(semicolons_v, sc6, 6);
            }
            if sc7 == 32 {
                masked_city7 = process_long(base.add(starts[7] as usize), &mut hash, &mut sc7);
                semicolons_v = _mm256_insert_epi32(semicolons_v, sc7, 7);
            }
        }

        let city_hashes = hash_cities(
            masked_city0,
            masked_city1,
            masked_city2,
            masked_city3,
        ...
        let low_words = _mm256_castps_si256(_mm256_shuffle_ps(
            _mm256_castsi256_ps(nums_low),
            _mm256_castsi256_ps(nums_high),
            0x88,
        ));
        let high_words = _mm256_castps_si256(_mm256_shuffle_ps(
            _mm256_castsi256_ps(nums_low),
            _mm256_castsi256_ps(nums_high),
            0xDD,
        ));

        let minus_mask = _mm256_cmpeq_epi8(
            low_words,
            _mm256_set1_epi16(';' as i16 | (('-' as i16) << 8)),
        );

        let nums_low_left1 = _mm256_slli_epi64(nums_low, 8);
        let nums_high_left1 = _mm256_slli_epi64(nums_high, 8);
        let high_words_left1 = _mm256_castps_si256(_mm256_shuffle_ps(
            _mm256_castsi256_ps(nums_low_left1),
            _mm256_castsi256_ps(nums_high_left1),
            0xDD,
        ));

        let mut nums_blended = _mm256_castps_si256(_mm256_blendv_ps(
            _mm256_castsi256_ps(high_words_left1),
            _mm256_castsi256_ps(high_words),
            _mm256_castsi256_ps(minus_mask),
        ));
        ...
        let h0 = insert_city(
            &mut hash,
            _mm256_extract_epi32(city_hashes, 0) as i64,
            masked_city0,
        );
        let v0 =
        ...
        let v4 =
            _mm_load_si128(hash.p.hashed_storage.add(4 * h4 as usize + 16 * 4) as *const __m128i);
        let h1 = insert_city(
            &mut hash,
            _mm256_extract_epi32(city_hashes, 4) as i64,
            masked_city1,
        );
        let v1 =
        ...
        let v5 =
            _mm_load_si128(hash.p.hashed_storage.add(4 * h5 as usize + 16 * 5) as *const __m128i);
        let h2 = insert_city(
            &mut hash,
            _mm256_extract_epi32(city_hashes, 1) as i64,
            masked_city2,
        );
        let v2 =
            _mm_load_si128(hash.p.hashed_storage.add(4 * h2 as usize + 16 * 2) as *const __m128i);
        let h6 = insert_city(
            &mut hash,
            _mm256_extract_epi32(city_hashes, 3) as i64,
            masked_city6,
        );
        let v6 =
            _mm_load_si128(hash.p.hashed_storage.add(4 * h6 as usize + 16 * 6) as *const __m128i);
        let h3 = insert_city(
            &mut hash,
            _mm256_extract_epi32(city_hashes, 5) as i64,
            masked_city3,
        );
        let v3 =
            _mm_load_si128(hash.p.hashed_storage.add(4 * h3 as usize + 16 * 3) as *const __m128i);
        let h7 = insert_city(
            &mut hash,
            _mm256_extract_epi32(city_hashes, 7) as i64,
            masked_city7,
        );
        let v7 =
            _mm_load_si128(hash.p.hashed_storage.add(4 * h7 as usize + 16 * 7) as *const __m128i);
        ...
        let abef_shift = _mm256_set_epi64x(
            0x0707070707060504,
            0x0303030303020100,
            0x0707070707060504,
            0x0303030303020100,
        ...
        let cdgh_shift = _mm256_set_epi64x(
            0x0F0F0F0F0F0E0D0C,
            0x0B0B0B0B0B0A0908,
            0x0F0F0F0F0F0E0D0C,
            0x0B0B0B0B0B0A0908,
        ...
        _mm_store_si128(
            hash.p.hashed_storage.add(4 * h2 as usize + 16 * 2) as *mut __m128i,
            _mm256_extracti128_si256(n_cg, 0),
        );
        _mm_store_si128(
            hash.p.hashed_storage.add(4 * h3 as usize + 16 * 3) as *mut __m128i,
            _mm256_extracti128_si256(n_dh, 0),
        );
        _mm_store_si128(
            hash.p.hashed_storage.add(4 * h4 as usize + 16 * 4) as *mut __m128i,
            _mm256_extracti128_si256(n_ae, 1),
        );
        _mm_store_si128(
            hash.p.hashed_storage.add(4 * h5 as usize + 16 * 5) as *mut __m128i,
            _mm256_extracti128_si256(n_bf, 1),
        );
        _mm_store_si128(
            hash.p.hashed_storage.add(4 * h6 as usize + 16 * 6) as *mut __m128i,
            _mm256_extracti128_si256(n_cg, 1),
        );
        _mm_store_si128(
            hash.p.hashed_storage.add(4 * h7 as usize + 16 * 7) as *mut __m128i,
            _mm256_extracti128_si256(n_dh, 1),
        );
    }
}
...
unsafe fn hash_cities(
    ...
    let acegbdfh = _mm256_castps_si256(_mm256_shuffle_ps(
        _mm256_castsi256_ps(acbd_xor),
        _mm256_castsi256_ps(egfh_xor),
        0x88,
    ));
...
unsafe fn insert_city(h: *mut Hash, mut hash: i64, masked_city: __m256i) -> i64 {
    loop {
        ...
        if _mm256_testz_si256(stored, stored) != 0 {
            _mm256_store_si256(
                (*h).p.hashed_cities.add(hash as usize) as *mut __m256i,
                masked_city,
            );
            *(*h).p.packed_offsets.add((*h).counts.num_cities as usize) = hash as i32;
            (*h).counts.num_cities += 1;

            let init_data = _mm256_set_epi32(
                MIN_TEMP,
                MAX_TEMP,
                (SUM_SIGN_BIT >> 32) as i32,
                0,
                MIN_TEMP,
                MAX_TEMP,
                (SUM_SIGN_BIT >> 32) as i32,
            ...
            _mm256_store_si256(
                (*h).p.hashed_storage.add(4 * hash as usize + 32) as *mut __m256i,
                init_data,
            );
            _mm256_store_si256(
                (*h).p.hashed_storage.add(4 * hash as usize + 64) as *mut __m256i,
                init_data,
            );
            _mm256_store_si256(
                (*h).p.hashed_storage.add(4 * hash as usize + 96) as *mut __m256i,
                init_data,
            );
            return hash;
        }
...
unsafe fn insert_city_long1(
    ...
    loop {
        let stored0 = _mm256_loadu_si256(
            (*hash).p.hashed_cities_long.add(hash_val as usize) as *const __m256i
        );
        let stored1 = _mm256_loadu_si256(
            (*hash).p.hashed_cities_long.add(hash_val as usize + 32) as *const __m256i
        );
        let xor0 = _mm256_xor_si256(stored0, seg0);
        let xor1 = _mm256_xor_si256(stored1, seg1);

        if _mm256_testz_si256(xor0, xor0) != 0 && _mm256_testz_si256(xor1, xor1) != 0 {
            return hash_val;
        }
        if _mm256_testz_si256(stored0, stored0) != 0 {
            _mm256_store_si256(
                (*hash).p.hashed_cities_long.add(hash_val as usize) as *mut __m256i,
                seg0,
            );
            _mm256_store_si256(
                (*hash).p.hashed_cities_long.add(hash_val as usize + 32) as *mut __m256i,
                seg1,
            );
...
unsafe fn insert_city_long2(
    ...
    loop {
        ...
        let stored2 = _mm256_loadu_si256(
            (*hash).p.hashed_cities_long.add(hash_val as usize + 64) as *const __m256i
        );
        let xor0 = _mm256_xor_si256(stored0, seg0);
        let xor1 = _mm256_xor_si256(stored1, seg1);
        let xor2 = _mm256_xor_si256(stored2, seg2);

        if _mm256_testz_si256(xor0, xor0) != 0
            && _mm256_testz_si256(xor1, xor1) != 0
            && _mm256_testz_si256(xor2, xor2) != 0
        {
            return hash_val;
        }
        if _mm256_testz_si256(stored0, stored0) != 0 {
            ...
            _mm256_store_si256(
                (*hash).p.hashed_cities_long.add(hash_val as usize + 64) as *mut __m256i,
                seg2,
            );
            (*hash).counts.num_cities_long += 1;
            return hash_val;
...
unsafe fn insert_city_long3(
    ...
    loop {
        ...
        let stored3 = _mm256_loadu_si256(
            (*hash).p.hashed_cities_long.add(hash_val as usize + 96) as *const __m256i
        );
        let xor0 = _mm256_xor_si256(stored0, seg0);
        let xor1 = _mm256_xor_si256(stored1, seg1);
        let xor2 = _mm256_xor_si256(stored2, seg2);
        ...
        if _mm256_testz_si256(xor0, xor0) != 0
            && _mm256_testz_si256(xor1, xor1) != 0
            && _mm256_testz_si256(xor2, xor2) != 0
            && _mm256_testz_si256(xor3, xor3) != 0
        {
            return hash_val;
        }
        if _mm256_testz_si256(stored0, stored0) != 0 {
            ...
            _mm256_store_si256(
                (*hash).p.hashed_cities_long.add(hash_val as usize + 96) as *mut __m256i,
                seg3,
            );
            (*hash).counts.num_cities_long += 1;
            return hash_val;
...
unsafe extern "C" fn sort_result(a: *const c_void, b: *const c_void, arg: *mut c_void) -> i32 {
    ...
    let left_bytes = if city_is_long(left_row.city) {
        (*(*r).long_cities.add(left_row.city.long_ref.index as usize))
            .bytes
            .as_ptr()
    } else {
    ...
    let right_bytes = if city_is_long(right_row.city) {
        (*(*r).long_cities.add(right_row.city.long_ref.index as usize))
            .bytes
            .as_ptr()
    } else {
    ...
    for i in 0..(*src).num_cities {
        ...
        if city_is_long(row_city) {
            ...
            while dst_long_idx < (*dst).num_long_cities {
                ...
                if long_city_equal(long_city, dst_long) {
                    break;
                }
                dst_long_idx += 1;
            }
        ...
        loop {
            ...
            if _mm256_testz_si256(xor, xor) != 0 {
                ...
                dst_row.min = if row.min < dst_row.min {
                    row.min
                } else {
                    dst_row.min
                };
                dst_row.max = if row.max > dst_row.max {
                    row.max
                } else {
                    dst_row.max
                };
            ...
            if _mm256_testz_si256(dst_row.city.reg, dst_row.city.reg) != 0 {
                *(*dst).refs.add((*dst).num_cities as usize) = ResultsRef {
                    offset: hash_val as u32,
                };
                *(*dst).rows.add(hash_val as usize / SHORT_CITY_LENGTH) = ResultsRow {
                    city: row_city,
                    ..row
                };
                (*dst).num_cities += 1;
...
unsafe fn long_city_equal(a: *const LongCity, b: *const LongCity) -> bool {
    ...
    _mm256_testz_si256(xor0, xor0) != 0
        && _mm256_testz_si256(xor1, xor1) != 0
        && _mm256_testz_si256(xor2, xor2) != 0
        && _mm256_testz_si256(xor3, xor3) != 0
}
...
unsafe fn city_from_long_hash(hash_val: i32) -> PackedCity {
    PackedCity {
        reg: _mm256_set_epi32(0, 0, 0, 0, 0, 0, hash_val, LONG_CITY_SENTINEL),
    }
}
...
unsafe fn convert_hash_to_results(hash: *mut Hash, out: *mut Results) {
    ...
    for i in 0..(*hash).counts.num_cities {
        ...
        let city_reg =
            _mm256_load_si256((*hash).p.hashed_cities.add(offset as usize) as *const __m256i);
        let mut city = PackedCity { reg: city_reg };
        let rows = (*hash)
            .p
            .hashed_storage
            .add(offset as usize * (HASH_ENTRY_SIZE / SHORT_CITY_LENGTH))
            as *mut HashRow;
        ...
        if city_is_long(city) {
            let long_city = &*((*hash)
                .p
                .hashed_cities_long
                .add(city.long_ref.index as usize)
                as *const LongCity);
        ...
        let mut res_offset =
            ((offset as u32 >> (HASH_SHIFT - HASH_RESULT_SHIFT)) & HASH_RESULT_MASK) as usize;
        loop {
        ...
        *(*out).refs.add(i as usize) = ResultsRef {
...
unsafe fn print_results(results: *mut Results) {
    ...
    for i in 0..(*results).num_cities {
        ...
        let row = &*(*results)
            .rows
            .add(ref_val.offset as usize / SHORT_CITY_LENGTH);

        let bytes = if city_is_long(row.city) {
            (*(*results).long_cities.add(row.city.long_ref.index as usize))
                .bytes
                .as_ptr()
        } else {
